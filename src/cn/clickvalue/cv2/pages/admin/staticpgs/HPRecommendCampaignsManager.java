...
public class HPRecommendCampaignsManager extends BasePage {
	...
	void setupRender() {
		hpBlock = hpBlockService.findUniqueBy("name", HPBlockEnum.RECOMMEND_CAMPAIGNS.name());
	...
	public BeanModel<HPBlockContent> getBeanModel() {
		...
		beanModel.get("cdata").label("广告活动").sortable(false);
	...
	void onValidateForm() {
		for (HPBlockContent hpBlockContent : hpBlockContents) {
			if (StringUtils.isEmpty(hpBlockContent.getCdata())) {
				form.recordError("广告活动不能为空");
				return;
			}
			if (campaignService.findCampaigns(hpBlockContent.getCdata(), true, null, 0, new Integer[] {}).size() == 0) {
				form.recordError(hpBlockContent.getCdata().concat("不存在或已删除"));
				return;
			}
		}
	}
	...
	Object onSubmit() {
		if (form.isValid()) {
			for (HPBlockContent hpBlockContent : hpBlockContents) {
				List<Campaign> campaigns = campaignService.findCampaigns(hpBlockContent.getCdata(), true, null, 0, new Integer[] {});
				hpBlockContent.setEntityId(campaigns.get(0).getId());
