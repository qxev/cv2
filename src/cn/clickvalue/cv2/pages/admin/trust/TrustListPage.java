...
public class TrustListPage extends BasePage {
	...
	public BeanModel<Campaign> getBeanModel() {
		...
		beanModel.add("affiliateCategories", null).label("与之匹配的网站类型").sortable(false);
		beanModel.add("operate", null).label("操作").sortable(false);
		beanModel.include("user.name", "name", "industrySubseries", "affiliateCategories", "operate");
		return beanModel;
	}
	...
	public String getAffiliateCategories() {
		StringBuilder sb = new StringBuilder();
		List<AffiliateCategoryCampaign> affiliateCategoryCampaigns = campaign.getAffiliateCategoryCampaigns();
		for (AffiliateCategoryCampaign affiliateCategoryCampaign : affiliateCategoryCampaigns) {
			AffiliateCategory affiliateCategory = affiliateCategoryCampaign.getAffiliateCategory();
			String name = affiliateCategory.getName();
	...
	public void onDelete(Integer campaignId) {
		Campaign camp = campaignService.get(campaignId);
		camp.setIntrust(0);
		campaignService.save(camp);
		affiliateCategoryCampaignService.deleteACSbyCampaignId(campaignId);
	}
