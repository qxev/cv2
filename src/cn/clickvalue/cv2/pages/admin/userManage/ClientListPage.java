...
public class ClientListPage extends BasePage {
    ...
    public String getVerified() {
        if (rowObj.getVerified() == null || rowObj.getVerified() != 2) {
            return "未审核";
        } else {
            return "已审核";
        }
    ...
    private void initQueryFromUser() {
        ...
        if(formVerified!=null) {
            if(formVerified==0) {
                c.addCriterion(Restrictions.or(Restrictions.ne("verified", 2), Restrictions.isNull("verified")));
            }else if(formVerified==2) {
                c.addCriterion(Restrictions.eq("verified", 2));
            }
        }
        if (formSite.getUser().getUserGroup().getId() != 0) {
            map.put("userGroup.id", getFormSite().getUser().getUserGroup()
                    .getId());
        }
    ...
    private void initBeanModelForUser() {
        ...
        beanModel.include("name", "userGroup.name", "verified", "actived",
    ...
    public Integer getFormVerified() {
        return formVerified;
    }

    public void setFormVerified(Integer formVerified) {
