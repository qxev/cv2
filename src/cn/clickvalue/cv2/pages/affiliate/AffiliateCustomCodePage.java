...
public class AffiliateCustomCodePage extends BasePage {
			...
			if (semClient == null) {
				...
				if (tracks != null && tracks.size() > 0) {
					for (int i = 0; i < tracks.size(); i++) {
						...
						tracker.addCustomAdvertising(model.getSiteId(), model
								.getSiteName(), model.getSiteUrl(), false,
								model.getAffId(), model.getAId(), model
										.getCampaignId(), model.getAdvHeight(),
								model.getAdvWidth(), model.getType(), model
										.getLandPageUrl(), model.getText(),
								model.getTargetWindow(), model.getPictureUrl(), campaign.getParameters());
					}
					addLog();
 					context = tracker.getCustomLink();
				} else {
