...
public class AffiliateEffectReport {
	@Property
	...
	void onActivate(Object... objects) {
		...
		this.sites = siteService.getSiteByUserId(getUser().getId());
		if (site == null) {
			site = new Site();
		}
		...
		if (startDate != null && endDate != null) {
			query.addCriterion(Restrictions.between("createTime", startDate, endDate));
			query1.addCriterion(Restrictions.between("createTime", startDate, endDate));
		}

		// 站点过滤
		if (site.getId() != null) {
			map.put("affiliateSiteId", site.getId().toString());
		}
