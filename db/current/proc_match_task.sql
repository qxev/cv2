		...
		if p_action_type = 0 or p_action_type = 1 then
			...
			if d_match_total > 0 then
				...
				if p_action_type = 1 then
					update `cv2`.`match_task` set exe_status = 3,check_success = d_program_cycle,match_success = d_match_success,program_cycle = d_program_cycle, check_message = 'Data scanning done.' where task_id = p_task_id;
				else
				...
				if d_no_confirm_data = 1 then
					...
					if d_task_match_action = 'match' then
						...
						/**update matching status**/
						update `cv2`.`report_trackmatch` set matched = 1,matchId = d_matched_id,siteCommisionNew=siteCommisionOld, darwCommisionNew=darwCommisionOld where campaignId = d_task_campaign_id and trackTime >= date_format(d_task_startdate,'%Y-%m-%d 00:00:00') and trackTime <= date_format(d_task_enddate,'%Y-%m-%d 23:59:59');
					end if;
				else
