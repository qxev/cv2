...
BEGIN
	...
	DECLARE cur_in CURSOR FOR SELECT advertiseAffiliateId,semId,advertiseId,trackCode,campaignId,affiliateId,siteId,subSiteId,dispalyType,dataType,sum(dataTotal) as dataTotal,ruleType,trackStatus,trackStep,orderinfo,trackIp,trackUser,referrerUrl,DATE_FORMAT(trackTime,'%Y-%m-%d 23:59:59') as trackTime,createdAt,updatedAt,orderid,orderamount FROM `cv2`.`affiliatecpmdata` where trackTime >= p_startday and trackTime < p_endday and trackStep = 105 and trackStatus = 1 group by trackCode,DATE_FORMAT( trackTime, '%Y-%m-%d 23:59:59' );
