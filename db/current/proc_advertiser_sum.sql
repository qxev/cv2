...
IF NOT done THEN
        ...
        END IF;
        IF p_type=100 THEN UPDATE `cv2`.advertiserSumInfo SET cpc=p_value,updatedAt=sysdate() WHERE campaignId=p_campaignId and userId=p_advertiserId;
        ELSEIF p_type=101 THEN UPDATE `cv2`.advertiserSumInfo SET cpl=p_value,updatedAt=sysdate() WHERE campaignId=p_campaignId and userId=p_advertiserId;
        ELSEIF p_type=102 THEN UPDATE `cv2`.advertiserSumInfo SET cps=p_value,updatedAt=sysdate() WHERE campaignId=p_campaignId and userId=p_advertiserId;
        ELSEIF p_type=105 THEN UPDATE `cv2`.advertiserSumInfo SET cpm=p_value,updatedAt=sysdate() WHERE campaignId=p_campaignId and userId=p_advertiserId;
        END IF;
END IF;
...
IF NOT done THEN
        IF p_advid IS NOT NULL THEN
                ...
                END IF;
                UPDATE `cv2`.advertiserSumInfo SET sites=p_sites,updatedAt=sysdate() WHERE campaignId=p_campId and userId=p_advid;
        END IF;
END IF;
UNTIL done END REPEAT;
CLOSE cur2;
