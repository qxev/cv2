...
DELIMITER $$
CREATE DEFINER=`cv2_production`@`192.168.1.%` PROCEDURE `cv2`.`proc_split_report_trackmatch`()
BEGIN
	...
	ALTER TABLE `report_trackmatch` RENAME TO `report_trackmatch_temp`;
	
 	CREATE TABLE `report_trackmatch` (
	  `id` int(11) NOT NULL,
 	...
 	call `cv2`.`proc_move_report_trackmatch`(d_split_date);
