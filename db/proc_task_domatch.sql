...
BEGIN
			...
			if d_continuedo = 1 then
					...
					/**order by**/
					set order_sql = concat(order_sql,' order by rand()*id asc');
					set ip_sql = concat(ip_sql,' order by rand()*id asc');
					set default_sql = concat(default_sql,' order by rand()*id asc');
					set final_sql = concat(final_sql,' order by rand()*id asc');
					...
					if @mrid > 0 then
							...
							if @rtype = d_effect_type then
									...
									SELECT advertiseAffiliateId,semId,advertiseId,trackCode,campaignId,affiliateId,siteId,subSiteId,dispalyType,dataType,dataTotal,ruleType,trackStatus,trackStep,orderinfo,trackIp,trackUser,referrerUrl,trackTime,createdAt,updatedAt,orderid,orderamount,country,city 
											into d_advertiseAffiliateId,d_semId,d_advertiseId,d_trackCode,d_campaignId,d_affiliateId,d_siteId,d_subSiteId,d_dispalyType,d_dataType,d_dataTotal,d_ruleType,d_trackStatus,d_trackStep,d_orderinfo,d_trackIp,d_trackUser,d_referrerUrl,d_trackTime,d_createdAt,d_updatedAt,d_orderid,d_orderamount,d_country,d_city
												 from `cv2_cmatch`.`report_trackmatch` where id = @mrid;
									...
									insert into `cv2_cmatch`.`affiliatetrackdata` (campaignId,semId,advertiseId,affiliateId,siteId,advertiseAffiliateId,ruleType,trackStep,trackIp,trackTime,dataType,createdAt) values(0,1,0,0,0,0,0,0,concat(date_format(sysdate(),'%Y%m%d%H%I%S'),' at ',d_data_id,' fix'),sysdate(),9,sysdate());
									set @cmridn = 0;
									...
									/**update task data status**/
									update `cv2_cmatch`.`match_data` set matched_done = 2, matched_message = 'SUCCESS', mapping_aid = @afid, mapping_id = @cmridn  where id = d_data_id;
