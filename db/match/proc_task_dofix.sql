...
DELIMITER $$
CREATE DEFINER=`root`@`localhost`  PROCEDURE `cv2`.`proc_task_dofix`(
...
BEGIN
	...
	if d_site_id > 0 then
			...
			if not exists(select 1 from `cv2`.`summarydimension` where advertiseAffiliateId = d_affilliate_advertise_id and campaignId = d_campaign_id) then
					set d_continuedo = 0;
