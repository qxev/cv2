drop procedure IF EXISTS `cv2_cmatch`.`proc_match_task`;
DELIMITER $$
CREATE PROCEDURE `cv2_cmatch`.`proc_match_task`(IN p_task_id bigint(20),IN p_action_type smallint(2))
BEGIN
	...
	DECLARE cur_in CURSOR FOR SELECT id,advertiser_id,campaign_id,advertis_id,affilliate_id,site_id,affilliate_advertise_id,subsite_id,effect_date,effect_ip,effect_cid,effect_aid,effect_type,effect_quantity,effect_amount,effect_commision,match_action,matched_check,matched_done,matched_message 
		FROM `cv2_cmatch`.`match_data` where task_id = p_task_id and matched_done = 0 and disabled = 0;
	...
	if exists(select 1 from `cv2_cmatch`.`match_task` where task_id = p_task_id) then 
		...
		/**reset task status**/
		update `cv2_cmatch`.`match_task` set exe_status = 1, check_message = '', check_success = 0,match_success = 0, match_total = 0,program_cycle = 0 where task_id = p_task_id;
		...
		if p_action_type = 0 or p_action_type = 1 then
			...
			update `cv2_cmatch`.`match_data` set matched_done = 0,matched_message = '', mapping_aid = 0, mapping_id = 0, matched_check = 0, disabled = 0 where task_id = p_task_id;
			...
			update `cv2_cmatch`.`match_task` set match_total = d_match_total where task_id = p_task_id;
			...
			if d_match_total > 0 then
				...
				if d_task_match_action = 'match' then
						...
						delete from `cv2_cmatch`.`report_trackmatch` where matchId = d_matched_id and dataType = 3;
						
						/**update matching status**/
						update `cv2_cmatch`.`report_trackmatch` set matched = 0,siteCommisionNew = 0,darwCommisionNew = 0  where matchId = d_matched_id;
				end if;
				...
				select count(1) into d_match_success from `cv2_cmatch`.`match_data` where task_id = p_task_id and matched_done > 0 and disabled = 0;
				
				update `cv2_cmatch`.`match_task` set exe_status = 2,check_success = d_program_cycle,match_success = d_match_success,program_cycle = d_program_cycle, check_message = 'Data scanning done.' where task_id = p_task_id;
			else
				...
				if d_no_confirm_data = 1 then
					...
					if d_task_match_action = 'match' then
						...
						/**update matching status**/
						update `cv2_cmatch`.`report_trackmatch` set matched = 1,matchId = d_matched_id,siteCommisionNew=siteCommisionOld, darwCommisionNew=darwCommisionOld where trackTime >= date_format(d_task_startdate,'%Y-%m-%d 00:00:00') and trackTime <= date_format(d_task_enddate,'%Y-%m-%d 23:59:59') and campaignId = d_task_campaign_id;
					end if;
				else
				...
				/**update task status**/
				update `cv2_cmatch`.`match_task` set exe_status = 3,check_success = d_match_total,match_success = d_match_total,check_message = d_check_message where task_id = p_task_id;
			end if;
		end if;
