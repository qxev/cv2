drop procedure IF EXISTS `cv2_cmatch`.`proc_commision_rule`;
DELIMITER $$
CREATE PROCEDURE `cv2_cmatch`.`proc_commision_rule`(
...
BEGIN
		...
		if d_ruleType >= 100 then
			if exists(select 1 from `cv2_cmatch`.`commissionrule` where campaignId = d_campaignId and ruleType = d_ruleType and verified = 2 and startDate <= d_trackTime and endDate >= d_trackTime) then
		...
		if p_confirm_mode = 1 then
				if d_effect_commision > 0 then
						if exists(select 1 from `cv2_cmatch`.`commissionladder` where campaignId = d_campaign_id and startDate <= @trkDate and endDate >= @trkDate and deleted = 0 and startCommission = d_effect_commision and isRange = 0) then
								...
								if exists(select 1 from `cv2_cmatch`.`commissionladder` where campaignId = d_campaign_id and startDate <= @trkDate and endDate >= @trkDate and deleted = 0 and startCommission <= d_effect_commision and endCommission >=  d_effect_commision and isRange = 1) then
											select d_effect_quantity*siteCommissionValue,d_effect_quantity*darwinCommissionValue into d_siteCommisionNew,d_darwCommisionNew 
														from `cv2_cmatch`.`commissionladder` 
