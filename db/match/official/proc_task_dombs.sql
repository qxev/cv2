drop procedure IF EXISTS `cv2`.`proc_task_dombs`;
DELIMITER $$
CREATE PROCEDURE `cv2`.`proc_task_dombs`(
...
BEGIN
			...
			DECLARE cpc_matched CURSOR FOR select data_id from `cv2`.`match_mbs_tmp` where match_id = d_matched_id and match_date = d_date_scope;
			...
			/**clear old data**/
			delete from `cv2`.`match_mbs_tmp` where match_id = d_matched_id and match_date = d_date_scope;
			
			set @cmatched_id = d_matched_id;
			set @cmatched_date = d_date_scope;
			/**create new data**/
			
			set @csql = 'insert into	`cv2`.`match_mbs_tmp`(';
			set @csql = concat(@csql,' select @cmatched_id,@cmatched_date,data_id from ');
			set @csql = concat(@csql,'(select a.id as data_id,rand()*a.id as rnd from `cv2`.`report_trackmatch` a where campaignId = ',d_campaign_id,' and trackTime >= \'',d_date_scope_start,'\' and trackTime <= \'',d_date_scope_end,'\' order by rnd asc) m');	
			...
			OPEN cpc_matched;
					REPEAT
						...
						IF NOT cpc_done THEN
								...
								/**update matched data**/
								update `cv2`.`report_trackmatch` set matched = 1,matchId = d_matched_id,siteCommisionNew = siteCommisionOld,darwCommisionNew = darwCommisionOld where id = d_cpc_id;
