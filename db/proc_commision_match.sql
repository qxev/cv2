...
DELIMITER $$
CREATE DEFINER=`cv2_production`@`192.168.1.%` PROCEDURE `cv2`.`proc_commision_match`(IN p_task_id bigint(20),IN p_type smallint(1))
BEGIN
	...
	/**DECLARE CONTINUE HANDLER FOR SQLEXCEPTION BEGIN
		insert into `cv2`.`report_task_result` (task_date,task_memo,task_result)
			values(sysdate(),concat('proc_commision_match(',p_task_id,',',p_type,',',d_data_id,')'),0);
	END;**/

	set AUTOCOMMIT = 1;

	/**check task exists or not**/
	if exists(select 1 from `cv2_cmatch`.`match_task` where task_id = p_task_id) then 
		/**clear old temp data**/
		delete from `cv2_cmatch`.`affiliatetrackdata` where dataType = 9;

		/**clear task status**/
		update `cv2_cmatch`.`match_task` set exe_status = 0, check_message = '', check_success = 0,match_success = 0, match_total = 0,program_cycle = 0 where task_id = p_task_id;

		/**read task information**/
		select task_startdate,task_enddate,no_confirm_data,campaign_id,task_name,task_date,user_id,match_total,check_success,check_failed,match_success,match_failed,check_message,match_message,exe_status,restart_task,match_id
			into d_task_startdate,d_task_enddate,d_no_confirm_data,d_task_campaign_id,d_task_name,d_task_date,d_user_id,d_match_total,d_check_success,d_check_failed,d_match_success,d_match_failed,d_check_message,d_match_message,d_exe_status,d_restart_task,d_matched_id
				from `cv2_cmatch`.`match_task` where task_id = p_task_id;
		...
		update `cv2_cmatch`.`match_data` set matched_done = 0,matched_message = '', mapping_aid = 0, mapping_id = 0, matched_check = 0 where task_id = p_task_id;

		/**mark the task status to start**/
		update `cv2_cmatch`.`match_task` set exe_status = 1 where task_id = p_task_id;

		select count(1) into d_match_total from `cv2_cmatch`.`match_data` where task_id = p_task_id and matched_done = 0;
		if d_match_total > 0 and d_matched_id <> '' then

			/**check task type and delete old data**/
			if p_type = 1 then
				/**insert into `cv2_cmatch`.`report_task_result` (task_date,task_memo,task_result) values(sysdate(),concat('delete old data:',d_matched_id,' ',2),0);
				really match data,clear old data of fixed, which data type value is 2**/
				delete from `cv2_cmatch`.`report_trackmatch` where matchId = d_matched_id and dataType = 2;
				commit;
			end if;
			
			update `cv2_cmatch`.`match_task` set check_message = 'Data scanning started .....' where task_id = p_task_id;
				...
				REPEAT
					...
					IF NOT done THEN
						...
						if d_check_success > 0 and d_check_counter = 0 then
							update `cv2_cmatch`.`match_task` set check_success = d_check_success,match_total = d_match_total where task_id = p_task_id;
						end if;
						if d_match_success > 0 and d_match_counter = 0 then
							update `cv2_cmatch`.`match_task` set match_success = d_match_success,match_total = d_match_total where task_id = p_task_id;
						end if;
						/**comp conditions**/
						set @mr = 0;
						...
						if d_effect_commision > 0 then
							if exists(select 1 from `cv2_cmatch`.`commissionladder` where campaignId = d_task_campaign_id and startDate <= @trkDate and endDate >= @trkDate and deleted = 0 and startCommission = d_effect_commision and isRange = 0) then
								select isRange,d_effect_quantity*siteCommissionValue,d_effect_quantity*darwinCommissionValue into d_isRange,d_siteCommissionValue,d_darwCommissionValue 
									from `cv2_cmatch`.`commissionladder` 
								...
								if exists(select 1 from `cv2_cmatch`.`commissionladder` where campaignId = d_task_campaign_id and startDate <= @trkDate and endDate >= @trkDate and deleted = 0 and startCommission <= d_effect_commision and endCommission >=  d_effect_commision and isRange = 1) then
									select isRange,d_effect_quantity*siteCommissionValue,d_effect_quantity*darwinCommissionValue into d_isRange,d_siteCommissionValue,d_darwCommissionValue 
										from `cv2_cmatch`.`commissionladder` 
								...
								if d_match_action = 'match' then
									...
									if d_effect_commision > 0 then
										if d_siteCommissionValue > 0 then
											update `cv2_cmatch`.`report_trackmatch` set matched = 1,matchId = d_matched_id,dataTotal = d_effect_quantity,siteCommisionNew = d_siteCommissionValue,darwCommisionNew = d_darwCommissionValue where id = @mrid;
										else
											update `cv2_cmatch`.`report_trackmatch` set matched = 1,matchId = d_matched_id where id = @mrid;
										end if;
									else 
										update `cv2_cmatch`.`report_trackmatch` set matched = 1,matchId = d_matched_id where id = @mrid;
									end if;
									...
									update `cv2_cmatch`.`match_data` set matched_done = 0, matched_message = 'EXCEPTION', mapping_aid = @afid, mapping_id = @mrid  where id = d_data_id;
								end if;
							else 
								/**not really do match, just update it to be checked and has found**/
								update `cv2_cmatch`.`match_data` set matched_check = 1, matched_message = 'RECORD FOUND', mapping_aid = @afid, mapping_id = @mrid where id = d_data_id;
								set d_check_success = d_check_success + 1;
							end if;
						else
							/**not found,fix the data, set data type is 2**/
							if p_type = 1 then
								if exists(select 1 from `cv2_cmatch`.`site` where id = d_site_id) then
									/**fix advertise id by default**/
									if not exists(select 1 from `cv2_cmatch`.`advertise` where id = d_advertis_id and campaignId = d_task_campaign_id and deleted = 0) then
										select id into d_advertis_id from `cv2_cmatch`.`advertise` where campaignId = d_task_campaign_id and deleted = 0 order by createdAt asc limit 0,1;
									end if;

									/**fix relationship between advertise and site**/
									if not exists(select 1 from `cv2_cmatch`.`advertiseaffiliate` where advertiseId = d_advertis_id and siteId = d_site_id)  then
										insert into `cv2_cmatch`.`advertiseaffiliate` (advertiseId,siteId,status,createdAt) values(d_advertis_id,d_site_id,2,sysdate());
										select last_insert_id() into d_affilliate_advertise_id;
									else
										if d_affilliate_advertise_id < 1 then
											select id into d_affilliate_advertise_id from `cv2_cmatch`.`advertiseaffiliate` where advertiseId = d_advertis_id and siteId = d_site_id order by createdAt asc limit 0,1;
										end if;
									end if;

									/***get new id**/
									insert into `cv2_cmatch`.`affiliatetrackdata` (campaignId,semId,advertiseId,affiliateId,siteId,advertiseAffiliateId,ruleType,trackStep,trackIp,trackTime,dataType,createdAt) values(0,1,0,0,0,0,0,0,concat(date_format(sysdate(),'%Y%m%d%H%I%S'),' at ',d_data_id),sysdate(),9,sysdate());
									set @cmrid = 0;
									...
									if d_ruleType >= 100 then
										/**insert into `cv2_cmatch`.`report_task_result` (task_date,task_memo,task_result) values(sysdate(),concat('type normal:',d_campaignId,' ',d_ruleType,' ',date(d_trackTime)),0);**/
										if exists(select 1 from `cv2_cmatch`.`commissionrule` where campaignId = d_campaignId and ruleType = d_ruleType and verified = 2 and startDate <= date(d_trackTime) and endDate >= date(d_trackTime)) then
											/**insert into `cv2_cmatch`.`report_task_result` (task_date,task_memo,task_result) values(sysdate(),'found commision',0);
											...
											select commissionType,ruleType,commissionValue,darwinCommissionValue 
												into d_commissionType,d_commissionRuleType,d_commissionValue,d_darwinCommissionValue 
													from `cv2_cmatch`.`commissionrule` 
									...
									if d_match_action = 'match' then
										update `cv2_cmatch`.`match_data` set matched_done = 2,matched_message = 'SUCCESS',effect_ip = d_effect_ip, mapping_aid = d_affilliate_advertise_id, mapping_id = @cmrid where id = d_data_id;
									else
										update `cv2_cmatch`.`match_data` set matched_done = 3,matched_message = 'SUCCESS',effect_ip = d_effect_ip, mapping_aid = d_affilliate_advertise_id, mapping_id = @cmrid where id = d_data_id;
									end if;

									set d_match_success = d_match_success + 1;
								else
									update `cv2_cmatch`.`match_data` set matched_check = 3,matched_message = 'SITE NOT FOUND' where id = d_data_id;
								end if;
							else
								/**not really match, just update to be checked and not found**/
								update `cv2_cmatch`.`match_data` set matched_check = 2,matched_message = 'RECORD NOT FOUND' where id = d_data_id;
								set d_check_success = d_check_success + 1;
							end if;
						end if;
					END IF;
			...
			update `cv2_cmatch`.`match_task` set exe_status = 2,check_success = d_check_success, check_message = 'Data scanning done.' where task_id = p_task_id;
		else
			/**allow to rebuild report**/
			select count(1) into d_match_total from `cv2_cmatch`.`report_trackmatch` where trackTime >= date_format(d_task_startdate,'%Y-%m-%d 00:00:00') and trackTime <= date_format(d_task_enddate,'%Y-%m-%d 23:59:59') and campaignId = d_task_campaign_id;
			if d_no_confirm_data = 1 then
				if p_type = 1 then
					update `cv2_cmatch`.`report_trackmatch` set matched = 1,matchId = d_matched_id where trackTime >= date_format(d_task_startdate,'%Y-%m-%d 00:00:00') and trackTime <= date_format(d_task_enddate,'%Y-%m-%d 23:59:59') and campaignId = d_task_campaign_id;
					update `cv2_cmatch`.`match_task` set match_total = d_match_total,match_success = d_match_total,check_success = d_match_total where task_id = p_task_id;
				else
					update `cv2_cmatch`.`match_task` set match_total = d_match_total,check_success = d_match_total where task_id = p_task_id;
				end if;
			else
				update `cv2_cmatch`.`match_task` set match_total = d_match_total,check_message = 'No confirmation data found, do nothing.' where task_id = p_task_id;
			end if;

			/**no any confirm data**/
			update `cv2_cmatch`.`match_task` set exe_status = 2,check_message = 'Data scanning done, No confirmation data found.' where task_id = p_task_id;
		end if;

		update `cv2_cmatch`.`match_task` set check_success = d_check_success,match_success = d_match_success,match_total = d_match_total,program_cycle = d_program_cycle where task_id = p_task_id;

		/**rebuild report for current campaign when really match data**/
		if p_type = 1 and d_match_total > 0 then
			update `cv2_cmatch`.`match_task` set check_message = 'Rebuilding report .....' where task_id = p_task_id;
			...
			call `cv2_cmatch`.`proc_create_daily_commision_by_campaign`(d_date_scope_start,d_date_scope_end,d_task_campaign_id);

			/**mark the task status to end**/
			update `cv2_cmatch`.`match_task` set exe_status = 3,check_message = 'Rebuilding report done.' where task_id = p_task_id;
		end if;
