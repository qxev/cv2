	...
	if p_campaignId = 0 then
		...
		REPEAT
			...
			IF NOT done THEN
				...
				if exists(select 1 from `cv2`.`report_trackmatch` where ruleType = d_ruleType and advertiseAffiliateId = d_advertiseAffiliateId and subSiteId = d_subSiteId and trackTime >= d_tmpSDate and trackTime <= d_tmpEDate limit 0,1) then
					if d_ruleType = 101 then
						if exists(select * from `cv2`.`report_trackmatch` where ruleType = d_ruleType and advertiseAffiliateId = d_advertiseAffiliateId and subSiteId = d_subSiteId and trackTime >= d_tmpSDate and trackTime <= d_tmpEDate and length(orderid) > 0 limit 0,1) then
							select if(sum(dataTotal) is null,0,sum(dataTotal)) as dataTotal,if(sum(orderamount) is null,0,sum(orderamount)) as orderamount,if(sum(siteCommisionOld) is null,0,sum(siteCommisionOld)) as siteCommisionOld,if(sum(darwCommisionOld) is null,0,sum(darwCommisionOld)) as darwCommisionOld 
								into d_dataTotalOld,d_orderamountOld,d_siteCommisionOld,d_darwCommisionOld 
									from (select * from `cv2`.`report_trackmatch` where ruleType = d_ruleType and advertiseAffiliateId = d_advertiseAffiliateId and subSiteId = d_subSiteId and trackTime >= d_tmpSDate and trackTime <= d_tmpEDate and length(orderid) > 0 group by subSiteId,advertiseAffiliateId,ruleType,orderid) t
						...
						select if(sum(dataTotal) is null,0,sum(dataTotal)) as dataTotal,if(sum(orderamount) is null,0,sum(orderamount)) as orderamount,if(sum(siteCommisionOld) is null,0,sum(siteCommisionOld)) as siteCommisionOld,if(sum(darwCommisionOld) is null,0,sum(darwCommisionOld)) as darwCommisionOld 
							into d_dataTotalOld,d_orderamountOld,d_siteCommisionOld,d_darwCommisionOld 
								from `cv2`.`report_trackmatch` 
									where ruleType = d_ruleType and advertiseAffiliateId = d_advertiseAffiliateId and subSiteId = d_subSiteId and trackTime >= d_tmpSDate and trackTime <= d_tmpEDate 
										group by subSiteId,advertiseAffiliateId,ruleType;
					end if;
				end if;

				if exists(select 1 from `cv2`.`report_trackmatch` where matched = 1 and ruleType = d_ruleType and advertiseAffiliateId = d_advertiseAffiliateId and subSiteId = d_subSiteId and trackTime >= d_tmpSDate and trackTime <= d_tmpEDate limit 0,1) then
					select if(sum(dataTotal) is null,0,sum(dataTotal)) as dataTotal,if(sum(orderamount) is null,0,sum(orderamount)) as orderamount,if(sum(siteCommisionNew) is null,0,sum(siteCommisionNew)) as siteCommisionNew,if(sum(darwCommisionNew) is null,0,sum(darwCommisionNew)) as darwCommisionNew 
						into d_dataTotalNew,d_orderamountNew,d_siteCommisionNew,d_darwCommisionNew 
							from `cv2`.`report_trackmatch` 
								where matched = 1 and ruleType = d_ruleType and advertiseAffiliateId = d_advertiseAffiliateId and subSiteId = d_subSiteId and trackTime >= d_tmpSDate and trackTime <= d_tmpEDate 
									group by subSiteId,advertiseAffiliateId,ruleType;
				end if;
